...
void tta_encoder_extend::init_set_info_for_memory(TTA_info* info, TTAuint64 pos) {
	...
	if (info->samples != MAX_SAMPLES)
	{
		...
		header_and_seektable_offset = offset + (frames + 1) * sizeof(TTAuint64);
	}
	else
	{
		header_and_seektable_offset = offset;
	}
...
void tta_encoder_extend::preliminaryFinish(void)
{
...
void tta_encoder_extend::flushFifo(void)
{
	writer_done(&fifo);
}
